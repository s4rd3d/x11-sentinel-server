...
-module(xss_events_rest_handler).
...
provide_json(Request, State) ->
    ...
    of
        undefined ->
            provide_events_json(Request, State);
        UserId ->
            provide_events_for_user_json(UserId, Request, State)
    end.

-spec provide_events_json(Request, State) -> {ResponseBody, Request, State} when
      Request :: cowboy_req:req(),
      State :: state(),
      ResponseBody :: iodata().
provide_events_json(Request1, State) ->
    {ok, Events} = xss_dashboard_api:select_events_aggregated_by_day(),

    % Set response headers
    Request2 = xss_utils:add_cors_headers(Request1),

    {jiffy:encode(#{result => ok,
                    events => lists:map(fun xss_utils:snake_to_camel/1,
                                               Events)}),
     Request2,
     State}.

-spec provide_events_for_user_json(UserId, Request, State) -> {ResponseBody, Request, State} when
      UserId :: xss_user:user_id(),
      Request :: cowboy_req:req(),
      State :: state(),
      ResponseBody :: iodata().
provide_events_for_user_json(UserId, Request1, State) ->
    {ok, Events} =
      xss_dashboard_api:select_events_by_user_id_aggregated_by_day(UserId),
